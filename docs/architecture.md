...
Consumer project (installed via `install <pack>` into a target repository)
  ...
  .sdd-agentic-flow/context/project-context.md     generated project facts (read-only, from context refresh/init)
...
| `project-context.ts` | `context status`/`refresh`, autonomy state |
...
- `conflicts` — optional list of skills that should not be installed together in the same pack.
  Empty by default.
- `requires_cli` — optional: the minimum `sdd-agentic-flow` CLI version
...
- `.sdd-agentic-flow/context/project-context.md`: auto-discovered facts (README, AI instruction files,
  ...
  `context refresh`, never hand-authored.
