...
| `config installation` | Save pack, skill scope, targets, and optional adoption intent; does not install skills |
| `config installation --plan` | Preview installation intent; never writes |
...
| `doctor --smoke` | Run the isolated setup and preservation smoke test |
| `doctor --contracts` | Validate installed skill capability contracts |
| `doctor --autonomy` | Validate autonomy configuration and guardrail inputs |
| `doctor --check-updates` | Run an opt-in, read-only update check |
...
| `context status` | Report context provenance and revision drift |
| `context autonomy-state` | Report policy and the latest autonomy loop state |
...
`version` prints the package identity. `completion bash|zsh|fish` prints a deterministic
completion script. `doctor --evidence-graph <slug>` is a read-only traceability projection;
add `--html` and optionally `--output <path>` for an HTML projection. `context refresh` is the
only public context mutation. Run `help <command>` before relying on a flag in automation.
...
- [TLC integration](tlc-integration.md)
