...
ADR directories, `package.json` identity and workspace declarations, monorepo tooling config, and
test config). It is distinct from `.sdd/config.yml`: config.yml holds user-declared policy, while
`project-context.md` holds discovered facts and is never written by hand.
