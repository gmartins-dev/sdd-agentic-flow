...
`shared/templates/handoff.template.md` already exists but no skill was required to populate it.
This file is the single generic statement of when and how a skill writes one, so continuity
across a session boundary or an agent swap does not depend on the invoking agent remembering to
ask for it.
...
continuity on its own, and a redundant `handoff.md` would only drift out of sync with it.
...
- `loop-state.md` (see [autonomy-guardrails.md](autonomy-guardrails.md)) is the mechanical,
  append-only execution-state ledger for one workflow run under `supervised`/`autonomous` —
  which skill ran, its `Status`, the proposed next skill, guardrail results.
- `handoff.md` is the human-readable continuity note for one feature or task — why the work
...
manual` (no `loop-state.md` in use) simply omits that reference and describes the state
directly.
...
Same posture as every other shared reference: this is a contract the skill and the invoking
agent honor, not something the CLI generates or enforces. `sdd-agentic-flow` ships no command
