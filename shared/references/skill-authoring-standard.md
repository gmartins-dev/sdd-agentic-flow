...
Every skill in `skills/` already follows the same six-section skeleton in practice. This
reference writes that skeleton down explicitly so a new skill starts from the documented
standard instead of by copying the closest existing `SKILL.md` and hoping the shape survives.
`scripts/check-skills.sh` mechanically enforces the six section headers and the frontmatter
contract fields; this document explains what belongs inside each one.
...
This is a content requirement inside the existing `## Output` section, not a seventh section — it
does not change the six-section contract that `scripts/check-skills.sh` already validates. A
skill keeps its own status vocabulary (`sdd-task-check` uses `pass`/`needs changes`/`blocked`/
