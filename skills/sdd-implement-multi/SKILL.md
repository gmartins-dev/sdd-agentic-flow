...
Use when a feature has multiple explicitly selected tasks and the user asks for a dependency-aware execution plan or orchestration. Read [the TLC baseline](../sdd-agentic-flow-shared/references/tlc-baseline.md) and [safety rules](../sdd-agentic-flow-shared/references/workflow-safety.md).
...
2. Resolve one feature, enumerate tasks, and build dependency waves from SDD evidence. Mark ambiguous or externally blocked tasks instead of guessing.
...
4. For each ready task, delegate or execute through `sdd-implement-task`; keep task scope and validation independent.
5. Collect results into a concise ledger and stop at blockers. Do not treat orchestration completion as feature validation or merge readiness.
