...
function autonomyComboValid(executionMode: string, autonomyLevel: string) {
...
const OPERATING_PRESETS: Record<string, { executionMode: string; autonomyLevel: string }> = {
...
const AUTONOMY_ALIASES: Record<string, string> = {
  man: 'manual',
  assist: 'supervised',
  assisted: 'supervised',
  auto: 'autonomous',
...
function resolveAutonomyToken(token: string) {
...
function resolveOperatingPreset(token: string) {
...
function sddJoin(cwd: string, ...segments: string[]) {
  return path.join(cwd, SDD_ROOT, ...segments);
}

function legacySddJoin(cwd: string, ...segments: string[]) {
...
const AGENT_USER_DIR_SEGMENTS: Record<string, string[][]> = {
  codex: [['.agents', 'skills']],
  cursor: [
    ['.agents', 'skills'],
    ['.cursor', 'skills'],
...
function userSkillsDirsFor(agent?: string | null, homeDir: string = os.homedir()) {
...
function userSkillsDirsForTargets(targets: string[], homeDir: string) {
  return [
    ...new Set(
...
function detectShellInfo(env: NodeJS.ProcessEnv = process.env) {
...
function configFor(options: CommandOptions = {}) {
...
function configValue(content: string, key: string) {
  const match = content.match(new RegExp(`^\\s+${key}:\\s*(.+)$`, 'm'));
  return match?.[1]?.trim().replace(/^['"]|['"]$/g, '') ?? null;
}

function resolveConfiguredAgent(cwd: string) {
...
function languageProfilePath(cwd: string, profile: string, isPackage: boolean) {
  return isPackage
...
function logPassLine(message: string, options: CommandOptions = {}) {
  const mode = options.mode ?? resolveMode(options);
  if (isRich(mode)) {
    process.stdout.write(`│\n`);
...
function readPreset(name: string): import('./upgrade').PresetLike | null {
  const filename = path.join(PRESETS_DIR, `${name}.json`);
  if (!fs.existsSync(filename)) return null;
  return JSON.parse(fs.readFileSync(filename, 'utf8')) as import('./upgrade').PresetLike;
}
...
function applyInitSideEffects(cwd: string, options: CommandOptions = {}) {
...
function init(cwd: string, options: InitOptions & CommandOptions = {}) {
...
function discoverProject(cwd: string, options: CommandOptions = {}) {
...
function parseProvenance(content: string): Record<string, unknown> | null {
...
function contextStatus(cwd: string) {
  const contextPath = sddJoin(cwd, 'context', 'project-context.md');
...
function contextRefresh(cwd: string, options: CommandOptions = {}) {
...
function autonomyStateReport(cwd: string) {
  const configPath = sddJoin(cwd, 'config.yml');
...
function autonomousResume(cwd: string, options: CommandOptions = {}) {
  const state = readLoopState(cwd);
  if (!state) {
    fail(
...
function setupLocationLabel(pack: string, scope: string, options: CommandOptions = {}) {
  return `${pack} ${isRich(resolveMode({ ascii: Boolean(options.ascii) })) ? '·' : '-'} ${scope}`;
}

function printSetupReview(draft: SetupDraft, locale: string, options: CommandOptions = {}) {
...
async function customizeSetup(draft: SetupDraft, locale: string, options: CommandOptions) {
...
async function applySetup(cwd: string, draft: SetupDraft, options: CommandOptions, locale: string) {
  ...
  if (draft.install) {
    ...
    if (
      !install(asString(draft.pack, 'core'), cwd, {
...
async function guidedInit(cwd: string, options: CommandOptions = {}) {
  const state = onboardingStateFor(cwd);
...
async function initInteractive(
  ...
  try {
    ...
    const options: InitOptions & InitInteractiveState = {
      language,
    ...
    options.name = await ask(t(locale, 'init.projectName'), 'example-project', null);
...
function printInstallNextSteps(cwd: string, options: CommandOptions = {}) {
  const mode = options.mode ?? resolveMode(options);
...
function install(pack: string, cwd: string, options: CommandOptions = {}) {
  const mode = resolveMode({ quiet: options.quiet, ascii: Boolean(options.ascii) });
  ...
  if (plan.blocked) {
    const legacy = plan.targets.some((target: TargetReport) => target.legacy);
    return fail(
      legacy
...
async function installInteractive(pack: string, cwd: string, options: CommandOptions = {}) {
  const mode = resolveMode({ quiet: options.quiet, ascii: Boolean(options.ascii) });
  const locale = localeFor(cwd);
  process.stdout.write(
...
function installationStatus(target: string) {
...
function hasCoreSkillsAt(root: string) {
...
function resolveSkillsRoot(cwd: string) {
...
function hasPrivateContext(paths: string[]) {
...
function severity(checks: DoctorCheck[]) {
...
function installedSkillDirs(root: string) {
...
function parseScalarField(frontmatter: string, field: string) {
...
function readLoopState(cwd: string) {
...
function skillOverrideLevel(content: string, skill: string) {
...
function autonomyCheck(cwd: string, options: CommandOptions = {}): InternalDoctorCheck[] {
  const checks: InternalDoctorCheck[] = [];
  const add = (
    name: string,
  ...
  if (options.verbose)
    for (const [name, message] of AUTONOMY_GUARDRAILS)
      add(asString(name), 'INFO', asString(message), 'Autonomy guardrails');

  return checks;
}

function renderDoctor(checks: DoctorCheck[], options: CommandOptions = {}) {
  const locale = asString(options.locale, 'en-US');
...
function smokeCheck(): InternalDoctorCheck {
  ...
  try {
    ...
    return {
      name: 'smoke',
      status: 'FAIL',
      message: `smoke failed; preserved for debugging: ${temporary} (${errorMessage(error)})`,
      section: 'Project readiness',
    };
  }
}

async function doctor(cwd: string, options: CommandOptions = {}) {
  ...
  if (options.json) process.stdout.write(`${JSON.stringify(result)}\n`);
    ...
    renderDoctor(checks, {
      mode: resolveMode({ json: options.json, ascii: Boolean(options.ascii) }),
...
function uninstall(args: string[], cwd: string): boolean | undefined {
...
function writeCommandHelp(key: string): void {
    ...
    if (args.includes('--help')) writeCommandHelp('init');
      ...
      for (let index = 0; index < args.length; index += 1) {
        if (args[index] === '--interactive') interactive = true;
          ...
          if (!EXECUTION_MODES.includes(asString(args[index + 1]))) {
            fail(usage, {
              reason: args[index + 1]
