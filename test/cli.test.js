...
function runIsolatedHome(args, cwd, homeDir, input) {
...
function listAllEntries(root, prefix = '') {
...
test('help, version, and list are available', () => {
  ...
  assert.equal(run(['version']).stdout.trim(), '1.9.1');
...
test('project-context.md carries provenance metadata, gracefully, outside a git repo', () => {
  ...
  assert.match(content, /> Generated by sdd-agentic-flow 1\.9\.1/);
...
test('doctor JSON is parseable and smoke is isolated', () => {
  ...
  assert.equal(report.version, '1.9.1');
...
test('doctor without --check-updates never includes an update_check row (opt-in only)', () => {
  ...
  assert.equal(
    report.checks.find((check) => check.name === 'update_check'),
    undefined,
  );
});
...
test('doctor --check-updates reports an available update via a local registry stub', (t) => {
  ...
  const report = JSON.parse(result.stdout);
  const updateCheck = report.checks.find((check) => check.name === 'update_check');
...
test('doctor --contracts fails deterministically when requires_cli is not satisfied', () => {
  ...
  assert.match(
    check.message,
    /sdd-create-specs: requires CLI >=99\.0\.0, installed CLI is 1\.9\.1/,
  );
...
test('init --execution-mode/--autonomy-level writes both fields, and an invalid combination fails without writing a config', () => {
  ...
  const config = fs.readFileSync(path.join(validCwd, '.sdd/config.yml'), 'utf8');
  ...
  assert.equal(fs.existsSync(path.join(invalidCwd, '.sdd/config.yml')), false);
...
test('doctor --autonomy validates config, the execution_mode x autonomy_level matrix, and skill support', () => {
  ...
  assert.equal(defaultReport.checks.find((c) => c.name === 'autonomy_config').status, 'PASS');
  assert.equal(defaultReport.checks.find((c) => c.name === 'autonomy_combo').status, 'PASS');
  assert.equal(defaultReport.checks.find((c) => c.name === 'autonomy_skills').status, 'PASS');
  assert.equal(defaultReport.checks.find((c) => c.name === 'autonomy_loop_state').status, 'INFO');
  assert.equal(
    defaultReport.checks.some((c) => c.name.startsWith('guardrail_')),
    false,
  );
  ...
  const guardrailChecks = verboseReport.checks.filter((c) => c.name.startsWith('guardrail_'));
  ...
  const comboCheck = invalidCombo.checks.find((c) => c.name === 'autonomy_combo');
...
test('doctor --autonomy on a pre-v1.8.0 config (missing fields) warns rather than fails', () => {
  ...
  const configCheck = report.checks.find((c) => c.name === 'autonomy_config');
...
test('context autonomy-state reports config, then reflects a recorded loop-state.md', () => {
  ...
  assert.match(notFound.stdout, /no \.sdd\/autonomy\/loop-state\.md found/);

  const loopStateDir = path.join(cwd, '.sdd/autonomy');
...
function writeMultiBlockLoopState(cwd) {
  const dir = path.join(cwd, '.sdd/autonomy');
...
test('workflow.skill_overrides lets a per-skill autonomy_level override the workflow default', () => {
  ...
  const afterCheck = after.checks.find((c) => c.name === 'autonomy_skills');
...
test('doctor --autonomy reports autonomy_combo FAIL (not a misleading PASS) when autonomy_level is an invalid explicit value', () => {
  ...
  const report = JSON.parse(run(['doctor', '--json', '--autonomy'], cwd).stdout);
  assert.equal(report.checks.find((c) => c.name === 'autonomy_config').status, 'FAIL');
  const comboCheck = report.checks.find((c) => c.name === 'autonomy_combo');
  assert.equal(comboCheck.status, 'FAIL');
...
test('--br and --en are aliases for --language pt-BR/en-US, last flag wins', () => {
  ...
  assert.match(fs.readFileSync(path.join(brCwd, '.sdd/config.yml'), 'utf8'), /profile: pt-BR/);

  const enCwd = fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-agentic-flow-en-'));
  assert.equal(run(['init', '--en'], enCwd).status, 0);
  assert.match(fs.readFileSync(path.join(enCwd, '.sdd/config.yml'), 'utf8'), /profile: en-US/);
  ...
  assert.match(
    fs.readFileSync(path.join(precedenceCwd, '.sdd/config.yml'), 'utf8'),
    /profile: pt-BR/,
  );
...
test('uninstall --full clears config and regenerable state, never .specs/features', () => {
  ...
  assert.ok(!fs.existsSync(path.join(cwd, '.sdd/config.yml')));
  assert.ok(!fs.existsSync(path.join(cwd, '.sdd/context/project-context.md')));
  assert.ok(!fs.existsSync(path.join(cwd, '.sdd/snapshots')));
  assert.ok(!fs.existsSync(path.join(cwd, '.sdd/reports')));
...
test('doctor reports an Installation section for both scopes with no false FAIL', () => {
  ...
  const installationChecks = before.checks.filter((check) => check.name.startsWith('installation'));
  assert.ok(installationChecks.length >= 2);
  assert.ok(installationChecks.every((check) => check.status !== 'FAIL'));
  assert.ok(
    installationChecks.some((check) =>
  ...
  const userInstalled = after.checks.filter(
    (check) => check.name.startsWith('installation_user_') && check.status === 'PASS',
  );
...
test('doctor and the bare-invocation screen both detect a partial core skill install', () => {
  ...
  assert.equal(doctorJson.checks.find((check) => check.name === 'skills').status, 'WARN');
...
test('doctor reports a Platform section that never FAILs on shell or git absence', () => {
  ...
  assert.equal(run(['init'], cwd).status, 0);
  const report = JSON.parse(run(['doctor', '--json'], cwd).stdout);
  const platformChecks = report.checks.filter((check) => check.name.startsWith('platform_'));
  assert.ok(platformChecks.length >= 4);
  assert.ok(platformChecks.every((check) => check.status !== 'FAIL'));
  assert.ok(platformChecks.some((check) => check.message.startsWith('OS: ')));
  assert.ok(platformChecks.some((check) => check.message === 'Filesystem writable'));
  assert.ok(platformChecks.some((check) => check.message.startsWith('Shell: ')));
  assert.ok(
    platformChecks.some(
      (check) => check.message === 'Git: available' || check.message === 'Git: not available',
    ),
  );
  fs.rmSync(cwd, { recursive: true, force: true });
});
...
test('golden flow: greenfield init -> install core -> copy spec artifacts -> doctor PASS', () => {
  ...
  const report = JSON.parse(run(['doctor', '--json'], cwd).stdout);
  assert.equal(report.checks.find((c) => c.name === 'artifact-contracts').status, 'PASS');
  assert.equal(report.checks.find((c) => c.name === 'evidence-first').status, 'PASS');
  assert.notEqual(report.status, 'FAIL');
...
test('golden flow: upgrading from a v0.8.0-shaped installation is safe under the new install default', () => {
  ...
  assert.equal(doctorReport.checks.find((c) => c.name === 'capability_contracts').status, 'PASS');
...
test('a real npm pack tarball installs and its extracted CLI passes init/discover/install/doctor', (t) => {
  ...
  try {
    ...
    assert.equal(runPacked(['version']).stdout.trim(), '1.9.1');
