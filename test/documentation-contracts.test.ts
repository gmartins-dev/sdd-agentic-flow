...
test('release documentation loader reads roadmap, changelog, and package version', () => {
  ...
  assert.equal(state.packageVersion, '7.13.0');
  assert.match(state.roadmap, /Current release: v7\.13\.0/);
  assert.match(state.roadmap, /^- \*\*v7\.13\.0:\*\*/m);
  assert.match(state.changelog, /^## 7\.13\.0/m);
});
